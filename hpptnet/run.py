...
from train_ignore_offset import train as train_ignore_offset
from train_four import train as train_four
import argparse
import json
import shutil
import model
...
def run(rank, world_size, device, config):
  ...
  if rank == 0:
    ...
    # inputs = torch.randn(1, 67456, device=device)
    # writer.add_graph(net.module, inputs)
  ...
  if ignore_offset_head:
    train_ignore_offset(net, train_config, log_config, logger, writer)
  elif ignore_velocity_head:
    train(net, train_config, log_config, logger, writer)
  else:
    train_four(net, train_config, log_config, logger, writer)
...
def main(args):
  ...
  devices = os.environ['CUDA_VISIBLE_DEVICES'].split(',') if 'CUDA_VISIBLE_DEVICES' in os.environ.keys() else ['0']
  processes = []
  for rank in range(world_size):
    p = mp.Process(target=run, args=(rank, world_size, 'cuda:' + devices[rank], config))
    processes.append(p)
